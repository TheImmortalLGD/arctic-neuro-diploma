...
if 'predict_btn' in locals() and predict_btn:
    try:
       ...
       # 2. ЦИКЛ ПРОГНОЗА (С ИНЕРЦИЕЙ)
        ...
        # Коэффициент инерции (0.8 означает, что мы сохраняем 80% структуры льда)
        # Это "успокаивает" нейросеть и убирает размытие.
        alpha = 0.75 
        
        for day in range(1, horizon + 1):
            # 1. Спрашиваем нейросеть: "Куда сдвинется лед?"
            pred_ai = model.predict(input_batch, verbose=0)
            
            # 2. Применяем инерцию (Data Assimilation)
            # Мы смешиваем: (Старый четкий кадр * 0.75) + (Новый прогноз * 0.25)
            # Это сохраняет четкие границы, но позволяет льду медленно дрейфовать.
            pred_stabilized = (input_batch * alpha) + (pred_ai * (1 - alpha))
            
            # 3. Убираем "серый шум" (все что слабее 0.1 обнуляем)
            pred_clean = tf.where(pred_stabilized > 0.1, pred_stabilized, 0.0)
            
            # Готовим вход для следующего дня
